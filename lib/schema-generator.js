...
function arrayToObject (arrayType) {
  var objectType = {};

  arrayType.forEach( function(item) {
    objectType = Object.assign(objectType, item)
  }, this);
...
class Schema {
  ...
  static isSchemaComponentTypeNumber (schemaComponent) {
    var isSchemaComponentTypeNumber = false;
    if (typeof schemaComponent.type != "undefined") {
      if ((schemaComponent.type == "number") || (schemaComponent.type == "integer"))
      isSchemaComponentTypeNumber = true
    } else
      if (typeof schemaComponent.anyOf != "undefined") {

        schemaComponent.anyOf.filter( function (schemaComponentAnyOfItem) {
          if (typeof schemaComponentAnyOfItem.type != "undefined")
            if ((schemaComponentAnyOfItem.type == "number") || (schemaComponentAnyOfItem.type == "integer")) {
              isSchemaComponentTypeNumber = true;
              return true;
            }
        }, this)
      ...
      return (isSchemaComponentTypeNumber)
  }
      ...
      if (typeof propertySourceContent != "undefined") 
        if (typeof propertyDefinition.propertyType != "undefined") {
          switch (propertyDefinition.propertyType) {
            case "element" :
              if (typeof propertySourceContent.facets != "undefined") {
  ...
  static getFacets (propertyDefinition, propertySourceContent, schemaComponent) {
            ...
            switch (facetItem) {
              case "enumeration" : {
                if (typeof facets.enum == "undefined") 
                  facets.enum = [];
                var isSchemaComponentTypeNumber = Schema.isSchemaComponentTypeNumber(schemaComponent);
                var facetValue;
                ...
                facetContainerItem[facetItem].forEach(function (facet) {
                  ...
                  if (isSchemaComponentTypeNumber)
                    facetValue = Number(facet.value)
                  else
                    facetValue = facet.value
                  //052 facets.enum.push(facet.value)
                  facets.enum.push(facetValue)
                }, this);
  ...
  getPropertyDefinition (definitionPropertyContainer) {
    ...
    class PropertyPrimitive {
      ...
      setFacets (propertyDefinition, propertySourceContent) {
        var facets = Schema.getFacets(propertyDefinition, propertySourceContent, this);
    ...
    class PropertyArray {
      ...
      setFacet (arrayDefinition, arraySourceContent) {
        ...
        var facets = Schema.getFacets(arrayDefinition, arraySourceContent, this);
    ...
    class PropertyObject {
      ...
      //053 setType (definitionObjectProperty) {}
      setType (propertyDefinition, propertyName, propertySourceContent) {
        if ((typeof propertyDefinition.type != "undefined") || (typeof propertyDefinition.propertyType != "undefined")) {
          var objectType = Schema.getType(propertyDefinition, propertyName, propertySourceContent);
          if (typeof objectType != "undefined")
            if (typeof objectType.anyOf != "undefined")
              this.anyOf = objectType.anyOf
            else
      ...
      //TO-DO: confirm whether the following should be updated to follow the Schema class pattern used by PropertyArray.setFacet 
      ...
      setProperties (definitionObjectProperty, schemaComponent) {
        ...
        if (SchemaComponent.isComponent(definitionObjectProperty, config.derived.w3cXmlTypeSchema))
          ...
          //053 definitionObjectProperty.properties.forEach 
          for (var definitionObjectPropertyItemName in definitionObjectProperty.properties) {
            schemaComponent.pedigree.push(schemaComponent.identifier);
            //053+ pattern from SchemaGenerator 2060
            var propertySchemaComponent = new SchemaComponent(definitionObjectProperty.properties[definitionObjectPropertyItemName], schemaComponent.sourceSchema, schemaComponent.pedigree, schemaComponent.config);
            propertySchemaComponent.generateSchema();
            //053+ pattern from getProperties 1185
            // this.properties = Object.assign(this.properties, getProperty( { [definitionObjectPropertyItemName] : definitionObjectProperty } ));
            this.properties = Object.assign(this.properties, { [definitionObjectPropertyItemName] : { "$ref" : propertySchemaComponent.schemaFileDestination } } );
            }
...
/* 049a        }
    ...
    function getPropertyTemplateType (definitionObjectProperty) {
      ...
      if (typeof definitionObjectProperty.type != "undefined") {
        ...
        if ((typeof definitionObjectProperty.propertyType != "undefined") &&(definitionObjectProperty.propertyType == "attribute")) {
    ...
    if (Array.isArray(definitionPropertyContainer)) 
      propertyContainer = arrayToObject(definitionPropertyContainer)
    else
      propertyContainer = definitionPropertyContainer;

    for (var definitionPropertyItemName in propertyContainer) {
      ...
      //053+
      if (definitionPropertyItemName != "enum") {
        var definitionPropertyItem;
        if (typeof propertyContainer.$ref != "undefined")
          definitionPropertyItem = propertyContainer
        else
          definitionPropertyItem = propertyContainer[definitionPropertyItemName];
        ...
        switch (propertyTemplateType) {
          ...
          case 'object':
            ...
            //053 propertyObject.setType(definitionPropertyItem);
            propertyObject.setType(definitionPropertyItem, definitionPropertyItemName, this.schemaComponent.sourceContent);
      ...
      }  //if (typeof definitionPropertyContainer.enum != "undefined")
    };
  ...
  getJavaType (propertyObject) {
    ...
    //053 var javaTypeParts = this.schemaComponent.schemaFileDestination.substring(0, this.schemaComponent.schemaFileDestination.lastIndexOf(".json"));
    var identifierParts = this.schemaComponent.identifier.replace(/^(https?):\/\//i, '');
    var identifierPartsDomain = identifierParts.substring(0, identifierParts.indexOf('/')).split('.');
    var identifierPartsDomainDescending = [];
    
    for (var item = identifierPartsDomain.length - 1; item >= 0; item--) {
      identifierPartsDomainDescending.push(identifierPartsDomain[item])
    };

    var identifierPartsPath = identifierParts.substring(identifierParts.indexOf('/'));
    var javaTypeParts = identifierPartsDomainDescending.join('/') + identifierPartsPath;
    //053- var javaTypePartsRelativePath = String(javaTypeParts.replace(/\.\.\//g, ''));
    //053 var javaTypePartsVersion = javaTypePartsRelativePath.replace(/(\d+\.?\d{0,9}?\.\d+\.?\d{0,9})/, replacer);
    var javaTypePartsVersion = javaTypeParts.replace(/(\d+\.?\d{0,9}?\.\d+\.?\d{0,9})/, replacer);
    //053 var javaType = String(javaTypePartsVersion.replace(/-?\./g, '_')).replace(/\//g, '.');
...
//050 may be deprecated by Schema.setType
  ...
  getObject () {
    ...
    Schema.getFacets(this.schemaComponent.definition, this.schemaComponent.sourceContent, this.schemaComponent));
